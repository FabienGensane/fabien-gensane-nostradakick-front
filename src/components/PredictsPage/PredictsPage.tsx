...
export const PredictsPage = () => {
	...
	const filteredMatchs = sortedMatchs.filter((match) =>{
		...
		if (match.team[0].name.includes(filteredValue.toLowerCase()) || match.team[1].name.includes(filteredValue.toLowerCase())) {
			return true;
		}
		return false
	});
	...
	return (
		<main className="main__predicts">
			<Search filteredValue={filteredValue} setFilteredValue={setFilteredValue} className="main__predicts__search" />
