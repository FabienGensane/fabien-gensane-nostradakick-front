...
const Predict_Card_logged = ({ match, initialPrediction }: PredictCardLoggedProps) => {
	...
	const createPredict = async (data: IPropsCreatePredict) => {
		try {
			...
			if (!createdPredict.prediction_id) {
				console.error("❌ ERREUR: prediction_id est undefined après création !");
				return;
			}
	...
	const handleDeletePredict = async (
		...
		try {
			...
			console.log("DELETE request URL:", `http://localhost:3000/api/predictions/${scorePredict?.prediction_id}`);
	...
	const handlePatchPredict = async (data: IPropsCreatePredict) => {
		...
		try {
			...
			console.log("PATCH request URL:", `http://localhost:3000/api/predictions/${scorePredict.prediction_id}`);
